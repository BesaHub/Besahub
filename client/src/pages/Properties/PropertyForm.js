...
const PropertyForm = () => {
  ...
  const validateForm = () => {
    ...
    if (formData.listingType === 'sale' && !formData.listPrice) {
      errors.listPrice = 'List price is required for sale listings';
    }
    
    if (formData.listingType === 'lease' && !formData.leaseRate) {
      errors.leaseRate = 'Lease rate is required for lease listings';
    }
    
    if (!formData.totalSquareFootage) {
      errors.totalSquareFootage = 'Total square footage is required';
    }
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      setSnackbar({
        open: true,
        message: `Property ${isEditing ? 'updated' : 'created'} successfully`,
        severity: 'success'
      });
      ...
      // Navigate back to properties list immediately (pass new property for optimistic add)
      ...
      const created = createdBase && uploadedImageUrls.length > 0
        ...
        : createdBase;
      navigate('/properties', { replace: true, state: { justCreated: !isEditing, newProperty: created } });

    } catch (err) {
      setSnackbar({
        open: true,
        message: `Failed to ${isEditing ? 'update' : 'create'} property`,
        severity: 'error'
      });
    } finally {
      setSaving(false);
      ...
      <Paper sx={{ p: 2 }}>
        <form onSubmit={handleSubmit}>
          <Grid container spacing={1.5}>
            ...
            <Grid item xs={12} md={6}>
              <FormControl fullWidth size="small" required error={!!formErrors.propertyType}>
