...
// Serve static files - serve React app if build exists
const clientBuildPath = path.join(__dirname, '../client/build');
const fs = require('fs');

if (fs.existsSync(clientBuildPath) && fs.existsSync(path.join(clientBuildPath, 'index.html'))) {
  // Serve static files from the React app build directory (CSS, JS, images, etc.)
  ...
  app.get('*', (req, res) => {
    ...
    if (req.path.startsWith('/api') || req.path === '/health') {
      // This shouldn't happen as API routes are defined before this
  ...
  // 404 handler if no build exists
  app.use('*', (req, res) => {
    // API routes are handled above, so this is for unknown routes
