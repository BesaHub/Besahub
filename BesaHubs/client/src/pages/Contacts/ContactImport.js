...
const ContactImport = () => {
  ...
  const handleFileUpload = async (files) => {
    ...
    try {
      ...
      // Auto-map headers
      const { mapping, unmappedHeaders: unmapped } = mapCSVHeaders(result.headers, CONTACT_FIELD_MAPPINGS);
  ...
  const proceedToReview = () => {
    const result = processCsvData(csvData.data, headerMapping, 'contact');
  ...
  const handleImport = async () => {
    ...
    try {
      ...
      for (let i = 0; i < validRows.length; i++) {
        try {
          await contactApi.createContact(validRows[i]);
          successCount++;
        } catch (error) {
          errorCount++;
          errors.push({
      ...
      <CardContent>
        <Box sx={{ textAlign: 'center', py: 4 }}>
          ...
          <Typography variant="h6" gutterBottom>
            Upload Contact CSV File
          </Typography>
          <Typography variant="body2" color="text.secondary" sx={{ mb: 3 }}>
            Upload a CSV file containing contact data to import into your system
          </Typography>
          ...
          <Button
            variant="outlined"
            startIcon={<Download />}
            onClick={() => downloadCSVTemplate('contact')}
            sx={{ mr: 2 }}
          >
            Download Template
          </Button>
          ...
          <Button
            ...
            onClick={() => setSnackbar({
              open: true,
              message: 'Template includes all supported contact fields with sample data',
              severity: 'info'
            })}
          >
            Template Info
  ...
  const renderMappingStep = () => (
    <Card>
      <CardContent>
        ...
        <Typography variant="body2" color="text.secondary" sx={{ mb: 3 }}>
          Map your CSV columns to the appropriate contact fields. Auto-mapped fields are shown below.
        </Typography>
          ...
          <Table>
            <TableHead>
              <TableRow>
                ...
                <TableCell>Maps to Contact Field</TableCell>
                  ...
                  <TableCell>
                    <FormControl size="small" sx={{ minWidth: 200 }}>
                      ...
                      >
                        ...
                        {Object.keys(CONTACT_FIELD_MAPPINGS).map(field => (
  ...
  const renderReviewStep = () => (
    <Box>
      ...
      <Box sx={{ display: 'flex', justifyContent: 'flex-end', gap: 2 }}>
        ...
        >
          Import {processedData?.validCount} Contacts
        </Button>
      </Box>
    </Box>
  );
    ...
    <Box>
      ...
      <Box sx={{ mb: 3 }}>
        ...
        <Typography variant="body1" color="text.secondary">
          Bulk import contact data from a CSV file
        </Typography>
      </Box>
